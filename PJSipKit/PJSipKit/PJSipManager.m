...
- (BOOL)initConfig {
    ...
    // udp transport
    {
        ...
        // 传输类型配置
        if ([PJSipUtil isIpv6Net]) {
            tp_type = PJSIP_TRANSPORT_UDP6;
        }else {
            tp_type = PJSIP_TRANSPORT_UDP;
        }
//        status = pjsua_transport_create(tp_type, &cfg, NULL);
        status = pjsua_transport_create(tp_type, &cfg, &tp_id);
        if (status != PJ_SUCCESS) {
...
//    cfg.reg_first_retry_interval = 60;
    ...
    if (status != PJ_SUCCESS) {
        ...
        model.state = PJ_SC_REGISTER_FAILED;
...
- (void)setRatioOfNormalVolumeLevel:(float)level {
...
//    NSLog(@"output : %d",output);
    ...
    pj_status_t status1 = pjsua_conf_adjust_rx_level(callInfo.conf_slot, level);
    pj_status_t status2 = pjsua_conf_adjust_tx_level(callInfo.conf_slot, level);
